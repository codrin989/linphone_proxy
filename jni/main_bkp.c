	...
	while (1) {
		...
		for (i = 0; i < NUM_FDS; i++) {
			...
			case 1:
				...
				count = rc;
				eprintf("SIP: from [LINPHONE: %d] - %d bytes\n",
			...
			case 2:
				...
				count = rc;
				eprintf("SIP: from [REMOTE: %d] - %d bytes\n",
				...
				printf("SIP: sent [LINPHONE] - %d bytes\n", count);
				break;
			...
			case 3:
				...
				count = rc;
				eprintf("DATA: from [LINPHONE: %d] - %d bytes\n",
			...
			case 4:
				...
				count = rc;
				eprintf("DATA: from [REMOTE: %d] - %d bytes\n",
			...
			case 5:
				...
				if (NORMAL == state && string_eq(buffer, "IP: ")) {
					...
					eprintf("MNGR: sent spoofed INVITE - %d bytes\n", rc);
					...
					eprintf("MNGR: sent spoofed ACK - %d bytes\n", rc);
					...
					state = ON_HOLD;
					eprintf("chaged state to ON_HOLD ...\n");
				}

				if (NORMAL == state && string_eq(buffer, "INVITE")) {
					eprintf("MNGR: received spoofed INVITE - %d bytes\n", count);
					...
					eprintf("MNGR: received spoofed ACK - %d bytes\n", rc);
					...
					eprintf("MNGR: sent spoofed INVITE to LINPHONE - %d bytes\n",
							spoofed_invite_len);
					...
					do {
						...
						count = rc;
						eprintf("MNGR: from [LINPHONE: %d] - %d bytes\n",
								htons(proxy_to_linphone_addr.sin_port), count);
					...
					eprintf("MNGR: sent spoofed ACK to LINPHONE - %d bytes\n",
							spoofed_invite_len);
					...
					eprintf("MNGR: sent confirmation\n");
				}
				...
				if (NORMAL == state && string_eq(buffer, "REDIRECT: ")) {
					...
					state = REDIRECTING;
					eprintf("chaged state to REDIRECTING ...\n");
				}
